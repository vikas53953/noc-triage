...
| [`copilot-cw13-netclaw-contract.md`](copilot-cw13-netclaw-contract.md) | Adopting NetClaw's ready-made MCP servers (first: catc-mcp): the vetting record, the connector seam (env boundary, redaction, pinned read-only record), PC set-up. | **Current — in flight** (PR #80). |
| [`copilot-cw14-runtime-contract.md`](copilot-cw14-runtime-contract.md) | Stop hand-rolling the agent loop: the runtime pick (OpenAI Agents SDK + provider adapter), what must survive, stages A–D. | **Current — pinned, not built.** |
