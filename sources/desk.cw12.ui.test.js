...
// ── 5. receipts: sent → picked up → answered, each a FACT ──────────────────
{
  ...
  ok('replied (ended on a narrowing question) says it is the operator\'s turn', /class="cw12-rcpt replied"/.test(CW9B.receiptHtml('replied')) && /asked you a question/.test(CW9B.receiptHtml('replied')));
...
// ── 8. the pages: wired, hidden when empty, never persisted ────────────────
{
  ...
  ok('desk: restored SENT / PICKED-UP ticks are removed (they imply an answer is coming); answered / replied / not-sent stay', /cw12SweepRestoredReceipts\(\);/.test(desk) && /\.cw12-rcpt:not\(\.answered\):not\(\.replied\):not\(\.not-sent\)/.test(desk));
